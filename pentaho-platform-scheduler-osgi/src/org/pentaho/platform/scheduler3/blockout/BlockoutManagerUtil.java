...
import org.pentaho.platform.api.scheduler3.CronJobTrigger;
...
public class BlockoutManagerUtil {
  ...
  public static boolean willFire( IBlockoutManager manager, IJobTrigger jobTrigger, List<IJobTrigger> blockOutTriggers, IScheduler scheduler ) {
    ...
    List<Date> fireTimes = getFireTimes( manager, jobTrigger, scheduler );
    ...
    for ( IJobTrigger blockOutJobTrigger : blockOutTriggers ) {
      ...
      if ( willBlockSchedule( manager, jobTrigger, blockOutJobTrigger, scheduler ) ) {
        ...
        if ( isBlockoutComplex ) {
          blockoutFireTimes = getFireTimes( manager, blockOutJobTrigger, scheduler );
        }
        ...
        for ( Date fireTime : fireTimes ) {
          scheduleCompletelyBlocked =
              isBlockoutComplex ? willComplexBlockOutTriggerBlockDate( blockOutJobTrigger, blockoutFireTimes, fireTime )
                  : willBlockDate( manager, blockOutJobTrigger, fireTime, scheduler );
  ...
  public static boolean willBlockSchedule( IBlockoutManager manager, IJobTrigger scheduleTrigger, IJobTrigger blockOutJobTrigger,
    ...
    if ( isScheduleTriggerComplex && isBlockOutTriggerComplex ) {
      return willComplexBlockOutBlockComplexScheduleTrigger( manager, blockOutJobTrigger, scheduleTrigger, scheduler );
    }

    // Complex Schedule Trigger
    if ( isScheduleTriggerComplex ) {
      return willBlockComplexScheduleTrigger( manager, scheduleTrigger, blockOutJobTrigger, scheduler );
    }

    // Complex BlockOut Trigger
    if ( isBlockOutTriggerComplex ) {
      return willComplexBlockOutTriggerBlockSchedule( manager, blockOutJobTrigger, scheduleTrigger, scheduler );
    }
  ...
  private static boolean willComplexBlockOutTriggerBlockSchedule( IBlockoutManager manager, IJobTrigger blockOutJobTrigger,
    ...
    for ( Date blockOutStartDate : getFireTimes( manager, blockOutJobTrigger, scheduler ) ) {
  ...
  private static boolean willBlockComplexScheduleTrigger( IBlockoutManager manager, IJobTrigger trigger, IJobTrigger blockOut,
      IScheduler scheduler ) {

    for ( Date fireTime : getFireTimes( manager, trigger, scheduler ) ) {
      if ( willBlockDate( manager, blockOut, fireTime, scheduler ) ) {
        return true;
      }
    }
  ...
  private static boolean willComplexBlockOutBlockComplexScheduleTrigger( IBlockoutManager manager, IJobTrigger blockOutJobTrigger,
      IJobTrigger jobTrigger, IScheduler scheduler ) {
    List<Date> blockOutFireTimes = getFireTimes( manager, blockOutJobTrigger, scheduler );

    int iStart = 0;
    for ( Date scheduleFireTime : getFireTimes( manager, jobTrigger, scheduler ) ) {
  ...
  private static boolean willBlockDate( IBlockoutManager manager, IJobTrigger blockOutJobTrigger, Date date, IScheduler scheduler ) {
    // S + Rx <= d <= S + Rx + D
    ...
    if ( isComplexTrigger( blockOutJobTrigger ) ) {
      return willComplexBlockOutTriggerBlockDate( blockOutJobTrigger, getFireTimes( manager, blockOutJobTrigger, scheduler ),
          date );
    }
  ...
  public static List<Date> getFireTimes( IBlockoutManager manager, IJobTrigger jobTrigger, IScheduler scheduler ) {
  ...
  public static boolean shouldFireNow( IBlockoutManager manager, List<IJobTrigger> blockOutJobTriggers, IScheduler scheduler ) {

    Date currentTime = new Date( System.currentTimeMillis() );
    for ( IJobTrigger blockOutJobTrigger : blockOutJobTriggers ) {

      if ( willBlockDate( manager, blockOutJobTrigger, currentTime, scheduler ) ) {
        return false;
      }
    }
  ...
  public static boolean isPartiallyBlocked( IBlockoutManager manager, IJobTrigger scheduleJobTrigger, List<IJobTrigger> blockOutJobTriggers,
      IScheduler scheduler ) {

    // Loop through blockout triggers
    for ( IJobTrigger blockOut : blockOutJobTriggers ) {
      if ( willBlockSchedule( manager, scheduleJobTrigger, blockOut, scheduler ) ) {
        return true;
      }
    }
